...
**Status (2026-09-25): decode and playback implementations are landed; the release
performance gate remains open.** The decode layer is implemented in `owe-media` and
proven against committed fixtures through the public API. The playback layer now has a
per-output clock, container timing, FPS caps, Wayland frame-callback presentation,
bounded frame allocation, replacement/clear/hotplug cancellation, play/pause/seek/loop
IPC, CLI and GUI controls, and decode/cache statistics. A live Hyprland probe applies
both the committed GIF and video fixtures, observes advancing frame counters, exercises
controls, clears the output, and shuts the daemon down cleanly.

The remaining P4 gate items are explicit rather than hidden: the design's dma-buf
zero-copy import is still a documented deviation, the ten-minute RSS and multi-cap
stability measurements are not yet published, and fuzz targets have not landed.

**Tasks**
- [x] `[T]` `AnimatedImageDecoder`: frame stream, timing from container; ring-buffer cache with `animated_frame_cap_mb` + zstd/lz4 eviction (unit tests with synthetic tiny GIFs; FR-LIVE-1). `crates/owe-media/src/animated.rs` (GIF/APNG/WebP through the `image` crate's frame iterator, a 100 ms floor on a declared zero delay) + `crates/owe-media/src/cache.rs` (`none|zstd|lz4`, hard byte cap). **71 unit tests + 8 public-API integration tests** — §4.1.
...
- [x] `[I]` Animated renderer and frame-clock presentation: decoded frames reach the GPU path, the compositor frame callback gates playback, and the live probe observes advancing GIF/video frames. The existing animated-frame render tests still pin pixels and fit modes.
...
- [x] `[T]` Playback IPC: play/pause/seek/loop idempotency, timeout/failure propagation, replacement, and state snapshots. `playback.cmd` and `stats.get` are covered by daemon, IPC, CLI, and GUI tests.
- [x] `[I]` Frame pacing integration: playback presents through the Wayland frame callback and the live `scripts/e2e-playback.sh` probe observes advancing GIF/video frames. The longer three-cap stability measurement remains part of the open release gate.
- [x] `[I]` GUI playback controls, decode-path badge, position/FPS/cache statistics, and fit-mode reporting.
...
**Exit gate — implementation is complete, but the performance release gate is still open.**
The live playback probe proves the feature path; the remaining unchecked items are
long-duration/resource measurements and security/fuzz coverage, not missing playback
wiring.

- [x] GIF and video playback, first-frame contract, pause/play, seek/loop IPC, statistics, clear, and clean shutdown pass in the live Hyprland probe.
- [x] Decode-path reporting is present in `stats.get` and the GUI; software decode is surfaced on the reference machine.
- [ ] 1080p video (H.264) plays 10 min on headless + HW: RSS within NFR-PERF-3 video budget ±10%, no unbounded growth (RSS graph filed).
- [ ] Frame pacing stability is green at caps 15/30/60 over repeated runs.
- [ ] 4K GIF forced into `streaming` mode at the default cap with RSS measured against cap plus overhead.
- [ ] Zero-copy dma-buf import is implemented or explicitly deferred by ADR; the current bounded SHM path is the documented fallback.
- [ ] Fuzz targets for IPC/config/shader metadata land with the security gate.
- [ ] PRD-F-14..16, FR-LIVE-1..6 green.

### 4.1 P4 evidence (measured on the reference machine, 2026-09-25)

| Gate item | Command | Observed result |
|---|---|---|
| Decode path, public API | `cargo test -p owe-media` | **71 unit + 8 integration tests, 0 failed.** The integration file (`tests/decode_path.rs`) goes through `open`/`open_kind`/`probe` only, against committed real files: `anim-3frame.gif` 32×32, 3 frames at 10 fps; `still.png` 2×2; `video-5frame.mp4` 64×48 H.264, 5 frames, 0.5 s at 10 fps |
...
| Fit mode (FR-LIVE-4) | `owed --check-config docs/examples/config.toml`, `cargo test -p owe-core -p owed --bin owed` | `config OK: docs/examples/config.toml (schema 1)` with `[render] fit = "fill"`; 177 core + 137 daemon tests passed. The engine test walks all four `KNOWN_FIT_MODES` strings and asserts each arrives at the renderer as itself, so a renamed mode cannot silently fall back to `fill` |
...
| Full workspace | `cargo test --workspace --all-targets --no-fail-fast` | **582 tests passed, 0 failed** across the workspace after the playback, media, IPC, shell, hotplug, and GUI fixes. |
| Live playback | `./scripts/e2e-playback.sh` | **PASS:** GIF advanced `11 -> 16` frames at `10.26 fps`; video advanced `8 -> 12` at `7.98 fps` on the live Hyprland session. Pause/play, seek, loop, clear, and daemon exit all succeed. |
| Capability surface | sandboxed `owed` + `owectl hello` | `content kinds: static-image, animated-image, video` when the video runtime is present; `media backends: image, gstreamer, ffmpeg`; shader remains explicitly unavailable. |
...
4. **`capabilities` answers two questions separately.** `content_kinds` is what `wallpaper.set` can present; `media_backends` is what this machine can decode. The distinction is preserved in the protocol and GUI, so a missing video runtime is reported rather than hidden.
5. **The P4 implementation is wider than the committed decode foundation.** Playback IPC, CLI/GUI controls, statistics, frame-callback presentation, lifecycle cancellation, and the live playback probe are now part of the worktree. The dma-buf zero-copy task and long-duration performance gate remain explicitly open rather than being presented as complete.
...
| P4 | 2026-09-25 | working tree (uncommitted) | **IMPLEMENTED; performance gate open** | Decode + playback, IPC, CLI/GUI controls, statistics, compositor frame callbacks, lifecycle cancellation, and the live `scripts/e2e-playback.sh` probe are present and passing. Zero-copy dma-buf, long-duration RSS/multi-cap measurements, and fuzz targets remain open. |
| P5 | — | — | not started | |
| P6 | — | — | not started | |
| P7 | — | — | not started | |
