...
14. **CI found four more things the desktop had masked** (runs 35461889214 / 35462429597 — this is precisely why gate item 1 stays open until CI is green):
    ...
    - The headless smoke script had a `#` comment **inside** a backslash-continuation chain; in bash that swallows the rest of the joined command, so `sway` never started and the log was empty. Comments now sit outside continuation chains, and `xwayland disable` in the headless config addresses the original CI-only Xwayland abort.
