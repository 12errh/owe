...
| ADR-017 | Caelestia in `shell-routed` mode is session-scoped: a named-output routed request is **refused**, not fanned out — the pinned CLI (`wallpaper-help.txt`) and IPC (`wallpaper.set(path)`) have no per-monitor target | ACCEPTED (P3) | Silently setting every monitor when one was asked for would report success for a request that was never honoured — the exact half-truth this project refuses. The refusal names the fix (`mode = "daemon-drawn"`), which has full per-output support. A test asserts the refusal against the pinned fixture, so a future shell release that *does* add a monitor argument fails the test and forces this ADR to be revisited rather than silently narrowed. |
